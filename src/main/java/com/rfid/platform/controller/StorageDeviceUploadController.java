...
import com.rfid.platform.persistence.storage.StorageDeviceUploadDetailRequestDTO;
...
public class StorageDeviceUploadController {
    ...
    public RfidApiResponseDTO<StorageDeviceUploadResponseDTO> uploadInbound(
        ...
        String orderNo = requestDTO.getData().getOrderNo();
        boolean oprtEnable = storageDeviceUploadRequestDTO.getOprtEnd();
        List<StorageDeviceUploadDetailRequestDTO> details = storageDeviceUploadRequestDTO.getDetails();

        if (oprtEnable && CollectionUtils.isEmpty(details)) {
            // 查询当前入库单中的箱数和rfids
            ...
            StorageDeviceUploadResponseDTO storageDeviceUploadResponseDTO = new StorageDeviceUploadResponseDTO();
            storageDeviceUploadResponseDTO.setOrderNo(orderNo);
            storageDeviceUploadResponseDTO.setOprtCnt(oprtCnt);
            storageDeviceUploadResponseDTO.setOprtBoxCnt(oprtBoxCnt);
            response.setData(storageDeviceUploadResponseDTO);

            if (!oprtCnt.equals(total)) {
                response.setStatus(false);
                response.setMessage("上传rfid总数与通知单总数不一致");
            } else {
                // 将入库单更新成完成状态
        ...
        if (CollectionUtils.isEmpty(details)) {
            response.setStatus(false);
            response.setMessage("上传数据明细不存在");
            return response;
        }

        for (StorageDeviceUploadDetailRequestDTO detail : details) {
            String productCode = detail.getProductCode();
            Integer rfidCnt = detail.getRfidCnt();

            // 1. 根据orderNo查出tagStorageOrderDetail中所有的productCode，过滤重复的值
            boolean existProductCode = tagStorageOrderDetailService.productCodeExistInOrderNo(orderNo, productCode);
            if (!existProductCode) {
                response.setStatus(false);
                response.setMessage("款式码不在通知单中");
                return response;
            }

            // 2. 根据orderNo和productCode查询tagStorageOrder中的quantity
            Integer quantity = tagStorageOrderDetailService.getQuantityFromTagStorageOrderDetails(orderNo, productCode);
            ...
            if (rfidCnt + existQuantity > quantity) {
                response.setStatus(false);
                response.setMessage("款式[" + productCode + "]上传数量大于入库通知单数量");
                return response;
            }
            ...
            if (oprtEnable && rfidCnt + existQuantity < quantity) {
                response.setStatus(false);
                response.setMessage("款式[" + productCode + "]上传数量小于入库通知单数量");
                return response;
            }

            // 3. 判断EPC在不在数据库
            List<String> epcs = detail.getRfids();
            ...
            if (CollectionUtils.isNotEmpty(invalidEpcs)) {
                ...
                response.setStatus(false);
                if (allMatch) {
                    response.setMessage("款式[" + productCode + "]上传的不在数据库中的rfid符合规则");
                } else {
                    response.setMessage("款式[" + productCode + "]上传的不在数据库中的rfid不符合规则");
                }
                return response;
            }
            ...
            if (!bindingCorrect) {
                response.setStatus(false);
                response.setMessage("款式码[" + productCode + "]和rfid的绑定关系不正确");
                return response;
            }
            ...
            if (!storageStateCorrect) {
                response.setStatus(false);
                response.setMessage("rfid库存状态不正确");
                return response;
            }
    ...
    @Operation(summary = "上传出库通知单扫描结果", description = "设备扫描EPC后上传出库结果")
    @PostMapping(value = "/uploadoutbound")
    @Transactional(rollbackFor = Exception.class)
    public RfidApiResponseDTO<StorageDeviceUploadResponseDTO> uploadOutbound(
            @ApiParam(value = "出库通知单扫描上传请求", required = true)
            @RequestBody RfidApiRequestDTO<StorageDeviceUploadRequestDTO> requestDTO) {
        RfidApiResponseDTO<StorageDeviceUploadResponseDTO> response = RfidApiResponseDTO.success();
        // 参数验证
        if (Objects.isNull(requestDTO) || Objects.isNull(requestDTO.getData())) {
        ...
        for (StorageDeviceUploadDetailRequestDTO detail : details) {
            ...
            List<TagStorageOrderDetailBean> orderDetails = tagStorageOrderDetailService.listTagStorageOrderDetails(orderNo);
            Integer total = orderDetails.stream()
                    .mapToInt(orderDetailBean -> orderDetailBean.getQuantity() != null ? orderDetailBean.getQuantity() : 0)
                    .sum();
            // 更新订单状态
            if (orderDetails.equals(total)) {
                // 操作结束，更新订单状态为完成(3)
                tagStorageOrderService.updateOrderStateByOrderNo(orderNo, requestDTO.getTimeStamp(), PlatformConstant.STORAGE_ORDER_STATUS.COMPLETED);
    ...
    @Operation(summary = "上传盘点通知单扫描结果", description = "设备扫描EPC后上传出库结果")
    @PostMapping(value = "/uploadinventory")
    @Transactional(rollbackFor = Exception.class)
    public RfidApiResponseDTO<StorageDeviceUploadResponseDTO> uploadInventory(
            @ApiParam(value = "盘点通知单扫描上传请求", required = true)
            @RequestBody RfidApiRequestDTO<StorageDeviceUploadRequestDTO> requestDTO) {
        RfidApiResponseDTO<StorageDeviceUploadResponseDTO> response = RfidApiResponseDTO.success();
        // 参数验证
        if (Objects.isNull(requestDTO) || Objects.isNull(requestDTO.getData())) {
